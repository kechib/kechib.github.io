// Shared internal auth for process-intake-submission.
...
function parseJwt(token: string): { header: JwtHeader; payload: JwtPayload; signingInput: string; signature: Uint8Array } | null {
...
function jwkToKey(alg: string, jwk: JsonWebKey): CryptoKey | null {
  const usages: KeyUsage[] = alg.startsWith("RS") || alg.startsWith("PS")
    ? ["verify"]
    : ["verify"];
  // EdDSA not expected here.
  try {
    return crypto.subtle.importKey("jwk", jwk, { name: "RSASSA-PKCS1-v1_5" } as AlgorithmIdentifier, false, usages)
      .then((k) => k)
      .catch(() => null) as unknown as CryptoKey;
  } catch {
    return null;
  }
}
...
async function verifyWithJwks(
  ...
  // alg:none and non-standard algs are never accepted.
...
export async function authorize(
  ...
  opts: {
    ...
    /**
     * When true (deployed with verify_jwt=true), accept a JWT whose payload
     * has role=service_role. Safe only because the platform has already
     * cryptographically validated the token signature before the handler runs.
     * Never enable this with verify_jwt=false.
     */
...
export function requirePost(req: Request): Response | null {
  ...
  if (req.method === "OPTIONS") return null; // CORS preflight handled by caller if needed

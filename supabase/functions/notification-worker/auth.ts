// Shared internal auth for notification-worker.
// NEVER trust a decoded JWT payload alone. Accept only:
...
//      with role=service_role (RS256/ES256 via Web Crypto), or
//   4) a platform-validated service_role payload (safe ONLY when the
//      function is deployed with verify_jwt=true — config.toml enforces it).
// Unauthenticated → 401. Service-only: no browser/anonymous callers.
...
function parseJwt(
  token: string,
): { header: JwtHeader; payload: JwtPayload; signingInput: string; signature: Uint8Array } | null {
